# ChatPDFv3 – MinerU-Powered PDF Intelligence with Knowledge Base

ChatPDFv3是一个命令行工作流，它可以将任何公开可下载的PDF链接或本地PDF文件利用[MinerU](https://mineru.net/)平台转换成Markdown格式文件，并进一步进行用户自定义的互动式快速解读，是对[Chatmd](https://github.com/ymphys/Chatmd)的升级。支持单一文件、批量处理，以及**跨文献知识库检索问答**模式。
...
1. **克隆仓库**
   ```bash
   git clone https://github.com/your-account/chatpdfv3.git
   cd chatpdfv3
   ```
...
2. **创建虚拟环境 (可选但推荐)**
   ...
   - 若需启用 HuggingFace/BGE 等本地 embedding 模型，请额外执行 `uv pip install ".[embeddings]"`。

3. **配置环境变量**
   - `OPENAI_API_KEY` – 默认使用 OpenAI Embedding API，为必填项
   - `DEEPSEEK_API_KEY` – 用于解读文档及知识库问答
   - `MINERU_API_KEY` – 用于上传PDF链接到 MinerU，如果你已经将PDF文件转换为markdown文件，则不需要获取该API key
   - 上述 API KEY 默认直接存储在环境变量中
...
**PDF URL 模式**
```bash
uv run chatpdf --pdf-url https://example.com/paper.pdf
```

**本地 Markdown 模式**
```bash
uv run chatpdf --md-path files/example.md
```
...
```bash
# 处理目录中的所有PDF文件
uv run chatpdf --batch-dir /path/to/pdf/files
```

**URL 批量处理**
```bash
# 处理文件中的PDF URL列表
uv run chatpdf --batch-urls-file files/batch_urls.txt

# 或者使用默认文件路径
uv run chatpdf --batch-urls-file
```

**批量任务状态查询**
```bash
# 查询批量任务状态
uv run chatpdf --batch-id your_batch_id_here
```
...
**自定义超时时间**
```bash
uv run chatpdf --batch-dir ./documents --mineru-timeout 1200
```

**选择模型版本**
```bash
uv run chatpdf --batch-dir ./documents --model-version vlm
```

**控制AI输出随机性**
```bash
uv run chatpdf --md-path document.md --temperature 0.7
```
...
新增加的 `kb` 子命令使 ChatPDFv3 能够把多篇论文整理为本地向量知识库，以供跨文档问答。

### 依赖与存储

- 默认已安装 `chromadb`。若想使用 HuggingFace/BGE 等本地 embedding 模型，请先执行 `uv pip install ".[embeddings]"`。
- 向量库会持久化在 `<files_root>/kb_store`（可通过 `--kb-path` 覆盖）。
- 构建流程自动调用 MinerU 将 PDF/URL 转为 markdown，再执行 chunk → embedding → ChromaDB 入库。
...
常用参数：
- `--kb-path`: 指定存储目录（默认：`files/kb_store`）
- `--chunk-size`: Markdown 切片长度（默认 800 字符）
- `--embedding-model`: OpenAI embedding 模型；若使用本地模型，配合 `--provider huggingface`
- `--mineru-model` / `--mineru-timeout`: 复用主流程的 MinerU 配置

### 向知识库提问

```bash
uv run chatpdf kb ask "这些文献如何处理束流色散？" --top-k 6 --save answers/dispersion.md
```

命令会：
1. 对问题向量化并检索最相关的片段
2. 将片段上下文交给 DeepSeek 生成答案
3. 通过 `--save` 可将问答写入 markdown

### 查看知识库状态

```bash
uv run chatpdf kb info
```

输出包括持久化路径、切片数量、所用 embedding 模型、最近更新时间等信息，方便确认知识库是否可用。
...
```bash
# 查询特定批次的结果
uv run chatpdf --batch-id 8c61f136-b2a0-4fa3-abd7-9a7f2a00fa61
```
